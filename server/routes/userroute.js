...
router.get('/:userid', (req, res) => {
  ...
  return res.json(
    MSG.generateErrorResponse('User not found', 'DATA_NOT_FOUND')
  );
});
...
router.post('/create', (req, res) => {
  ...
  if (usernames.includes(username)) {
    // Generate response
    return res.json(
      MSG.generateErrorResponse('User Exists', 'DATA_NOT_UNIQUE')
    );
  }
...
router.post('/subscribe/:subscriber/:account', (req, res) => {
  ...
  if (!subscriber || !account)
    return res.json(
      MSG.generateErrorResponse(
        'could not subscribe',
        'ARGS_MISSING_PARAMETERS'
      )
    );
  ...
  const users = userData.filter(
    user => user.username === subscriber || user.username === account
  );
  ...
  if (!accountData || !subscriberData)
    return res.json(
      MSG.generateErrorResponse('could not subscribe', 'DATA_NOT_FOUND')
    );
  ...
  if (Array.isArray(accountData.followers)) {
    ...
    if (accountData.followers.includes(subscriber))
      return res.json(
        MSG.generateErrorResponse(
          'could not subscribe',
          'DATA_REDUNDANT_ACCOUNT_ALREADY_FOLLOWED'
        )
      );
...
router.post('/unsubscribe/:subscriber/:account', (req, res) => {
  ...
  const subscriberFollowIndex =
    accountData.followers.indexOf(subscriber) != -1
      ? accountData.followers.indexOf(subscriber)
      : null;
  const accountFollowingIndex =
    subscriberData.following.indexOf(account) != -1
      ? subscriberData.following.indexOf(account)
      : null;
  if (subscriberFollowIndex === null)
    return res.json(
      MSG.generateErrorResponse(
        'User not subscribed',
        'LOGIC_ERROR_USER_NOT_SUBSCRIBED'
      )
    );
