...
router.get("/history/:userid", (req, res) => {
    ...
    let tweetsByUser = tweets.filter(tweet => tweet.username === username);
    // Get user displayName and add to tweet
    const [user] = users.filter(user => user.username === username);
    console.log(user);
    if (user) {
        tweetsByUser = tweetsByUser.map(tweet => {
...
router.get("/:tweetid", (req, res) => {
    ...
    const users = JSON.parse(fs.readFileSync("./data/users.json", "utf-8"));
    const [user] = users.filter(tweet => (tweet.username = username));
    // If tweet doesn't exist - chill
    if (!tweet)
        res.json(
            MSG.generateErrorResponse("Tweet not found", "DATA_NOT_FOUND")
        );
...
router.post(
    ...
    (req, res) => {
        ...
        const tweets = JSON.parse(
            fs.readFileSync("./data/tweets.json", "utf-8")
        );
